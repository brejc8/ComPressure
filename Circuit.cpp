...
SDL_Rect CircuitElementPipe::getimage_bg(void)
{
    if (connections == CONNECTIONS_NS)
    {
        ...
        return SDL_Rect{256 + x, 144 + int(offset), 6, 14};
    }
    if (connections == CONNECTIONS_EW)
    {
        ...
        return SDL_Rect{304 + int(offset), 144 + y, 14, 6};
    }
...
void CircuitElementPipe::sim_pre(PressureAdjacent adj)
{
    ...
    switch(connections)
    {
        ...
        case CONNECTIONS_NS:
        {
            Pressure mov = (adj.S.value - adj.N.value) / 4;
        ...
        case CONNECTIONS_EW:
        {
            Pressure mov = (adj.E.value - adj.W.value) / 4;
            adj.E.move(-mov);
            adj.W.move(mov);
...
void CircuitElementSource::sim_pre(PressureAdjacent adj_)
{
    ...
    adj.N.move((100 * PRESSURE_SCALAR - adj.N.value) / 4);
    adj.E.vent();
    adj.S.vent();
    adj.W.vent();
}
